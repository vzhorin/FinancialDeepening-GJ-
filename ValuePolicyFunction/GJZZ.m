function[ZZ]=GJZZ(sf, GJbet, GJdel, GJq, GJA)

% given sf=(save,phi), return -Z
% k and other parameters are exogenous

global SSS FFF h1 h2 h3 h4 f g  GJKJ;
...
ZK=zeros(15,1);
for i=1:15
     ZK(i)=wp(x(i), GJbet, GJdel, GJq, GJA);
end
...
if penal<=-100000000
	zz=penal;
        ...
        %intZ=quadlg('wpf1',h1,h2)+quadlg('wpf2',h2,h3)+quadlg('wpf3',h3,h4);
	intZ=(W.*pdf)*ZK;
	zz=log((1-SSS)*GJKJ)+GJbet*intZ;
end
ZZ=-zz;
