...
extern mclComponentData __MCC_gj_collect_component_data;
 ...
 */
#ifndef LIB_gj_collect_C_API 
#define LIB_gj_collect_C_API /* No special import/export declaration */
#endif

LIB_gj_collect_C_API 
bool MW_CALL_CONV gj_collectInitializeWithHandlers(
...
)
{
  ...
  if (!mclInitializeComponentInstance(&_mcr_inst,
                                      &__MCC_gj_collect_component_data,
...
LIB_gj_collect_C_API 
bool MW_CALL_CONV gj_collectInitialize(void)
{
  return gj_collectInitializeWithHandlers(mclDefaultErrorHandler,
                                          mclDefaultPrintHandler);
}

LIB_gj_collect_C_API 
void MW_CALL_CONV gj_collectTerminate(void)
{
  if (_mcr_inst != NULL)
...
int run_main(int argc, const char **argv)
{
  ...
  __MCC_gj_collect_component_data.path_to_component = path_to_component; 
  if (!gj_collectInitialize()) {
    return -1;
  }
...
int main(int argc, const char **argv)
{
  if (!mclInitializeApplication(
    __MCC_gj_collect_component_data.runtime_options,
    __MCC_gj_collect_component_data.runtime_option_count))
    return 0;
