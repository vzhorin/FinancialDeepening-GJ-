...
extern mclComponentData __MCC_gj_main_component_data;
 ...
 */
#ifndef LIB_gj_main_C_API 
#define LIB_gj_main_C_API /* No special import/export declaration */
#endif
...
)
{
  ...
  if (!mclmcrInitialize())
    return false;
  if (!mclInitializeComponentInstance(&_mcr_inst, &__MCC_gj_main_component_data,
...
LIB_gj_main_C_API 
bool MW_CALL_CONV gj_mainInitialize(void)
{
  return gj_mainInitializeWithHandlers(mclDefaultErrorHandler,
                                       mclDefaultPrintHandler);
}

LIB_gj_main_C_API 
void MW_CALL_CONV gj_mainTerminate(void)
{
  if (_mcr_inst != NULL)
