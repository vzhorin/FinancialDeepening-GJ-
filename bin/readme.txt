    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_gj_transition_a64_v78.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * gj_transition_a64_v78.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * gj_transition_a64_v78.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * gj_transition_a64_v78 (application created by MATLAB Compiler)
        * run_gj_transition_a64_v78.sh (shell script that can be run to 
    ...
    All Platforms:
        * gj_transition_a64_v78.so (shared library, file extension varies by 
          platform)
        * gj_transition_a64_v78.h (library header file)
        * gj_transition_a64_v78.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * gj_transition_a64_v78.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * gj_transition_a64_v78.xml (documentation files)
        * gj_transition_a64_v78.pdb (program database file - if DEBUG option 
          selected)
        * gj_transition_a64_v78.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * gj_transition_a64_v78.ctf (Component Technology File archive, contains 
          compiled M-code)
        * gj_transition_a64_v78_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * gj_transition_a64_v78.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * gj_transition_a64_v78.ctf (Component Technology File archive, contains 
          compiled M-code)
        * gj_transition_a64_v78.xla (Optionally created and placed in
          bin directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
