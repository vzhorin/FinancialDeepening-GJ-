...
extern mclComponentData __MCC_gj_collect_a64_v78_component_data;
 ...
 */
#ifndef LIB_gj_collect_a64_v78_C_API 
#define LIB_gj_collect_a64_v78_C_API /* No special import/export declaration */
#endif

LIB_gj_collect_a64_v78_C_API 
bool MW_CALL_CONV gj_collect_a64_v78InitializeWithHandlers(
...
)
{
  ...
  if (!mclInitializeComponentInstanceWithEmbeddedCTF(&_mcr_inst,
                                                     &__MCC_gj_collect_a64_v78_component_data,
                                                     true, NoObjectType,
                                                     ExeTarget, error_handler,
                                                     print_handler, 2011052, (void *)(gj_collect_a64_v78InitializeWithHandlers)))
    return false;
  return true;
}

LIB_gj_collect_a64_v78_C_API 
bool MW_CALL_CONV gj_collect_a64_v78Initialize(void)
{
  return gj_collect_a64_v78InitializeWithHandlers(mclDefaultErrorHandler,
                                                  mclDefaultPrintHandler);
}

LIB_gj_collect_a64_v78_C_API 
void MW_CALL_CONV gj_collect_a64_v78Terminate(void)
{
  if (_mcr_inst != NULL)
...
int run_main(int argc, const char **argv)
{
  ...
  __MCC_gj_collect_a64_v78_component_data.path_to_component = path_to_component; 
  if (!gj_collect_a64_v78Initialize()) {
    return -1;
  }
  _retval = mclMain(_mcr_inst, argc, argv, "gj_collect", 0);
  if (_retval == 0 /* no error */) mclWaitForFiguresToDie(NULL);
  gj_collect_a64_v78Terminate();
...
int main(int argc, const char **argv)
{
  if (!mclInitializeApplication(
    __MCC_gj_collect_a64_v78_component_data.runtime_options,
    __MCC_gj_collect_a64_v78_component_data.runtime_option_count))
    return 0;
