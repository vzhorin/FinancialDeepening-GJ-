...
% Subgroups with heterogeneous agents 
% ALSO NEED TO select initial participation at inipart!!!
%load gjpdlog76Rur; % inital participation 4.8%
%load gjpdlog76Urb; % 16.1%
%load gjpdlog76Ledu; % 5.3%
load gjpdlog76Hedu; % 20.2%
...
LZK = 1 - inipart/100 - 0.001; % 1- initial participation -0.001

%CGK=GJk(ZK);
%zk=asorder(kgrid,LZK);
zk=asorder(cd76,LZK);
%m=zk-1; % IMFWP version with 800 Need to check with initial participation
m=zk;
ZK=exp(kgrid(m))
...
save HISTHedu scale CAPI CONS SAV H T kgrid cgrid sgrid ....
